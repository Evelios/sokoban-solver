...
def breadth_first_search_v0(problem):
...
def iterative_deepening_astar(problem, limit=sys.maxint):
    def recursive_dl_astar(node, problem, limit):
        g = node.path_cost
        h = problem.h(node)
        f = g + h
        if f > limit:
        ...
        min = float('inf')
        for child in node.expand(problem):
            t, r_node = recursive_dl_astar(child, problem, limit)
            if t == 'found':
                return 'found', r_node
            if t < min:
                min = t
    ...
    ## Body of iterative_deepening_astar
    bound = problem.h(start_node)

    while bound < limit:
        bound, node = recursive_dl_astar(start_node, problem, bound)
        if bound == 'found':
            return node
